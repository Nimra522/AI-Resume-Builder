...
export const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const { pathname, navigate, search } = useLocation();
  const { unreadCount } = useNotifications();
  const { user, logout } = useAuth();
  const notificationRef = useRef<HTMLDivElement>(null);
  useClickOutside(notificationRef, () => setIsNotificationsOpen(false));

  // Check if we're on the resume editor page (has ?template or ?edit query params)
  const isEditorPage = pathname === '/dashboard' && (search.includes('template=') || search.includes('edit='));
  ...
  return (
    <div className="h-screen bg-slate-50 flex font-sans overflow-hidden">
      ...
      {!isEditorPage && <FloatingChatButton />}
    </div>
  );
};
