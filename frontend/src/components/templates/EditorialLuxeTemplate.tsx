  ...
  return (
    <div className="w-full min-h-[1122px] bg-[#FCFAF7] text-[#1F1F1F] font-['Inter',sans-serif] shadow-xl mx-auto" style={{ maxWidth: '793px' }}>
      ...
      <div className="px-10 pt-8 pb-4">
        <div className="flex items-start justify-between">
          ...
          <div className="w-[68px] h-[68px] overflow-hidden border border-[#D6C4AF] bg-[#EEE6DD] flex items-center justify-center flex-shrink-0 ml-6">
            {personalInfo.photoUrl ? <img src={personalInfo.photoUrl} alt="" className="w-full h-full object-cover" /> : <User size={26} className="text-[#D6C4AF]" />}
          </div>
        </div>

        {/* Contact — horizontal editorial spread */}
        <div className="flex gap-6 mt-6 pt-4 border-t border-[#EEE6DD]">
          {personalInfo.phone && <div className="flex items-center gap-1.5 text-[12px] text-[#555555] tracking-[0.08em] uppercase"><Phone size={9} className="text-[#D6C4AF]" />{personalInfo.phone}</div>}
          {personalInfo.email && <div className="flex items-center gap-1.5 text-[12px] text-[#555555] tracking-[0.08em] uppercase"><Mail size={9} className="text-[#D6C4AF]" /><span className="break-all">{personalInfo.email}</span></div>}
          {personalInfo.location && <div className="flex items-center gap-1.5 text-[12px] text-[#555555] tracking-[0.08em] uppercase"><MapPin size={9} className="text-[#D6C4AF]" />{personalInfo.location}</div>}
        </div>
      </div>
      ...
      {personalInfo.summary && (
        <div className="px-10 py-6">
          <div className="flex items-center gap-3 mb-3">
            <span className="text-[11px] font-['Cormorant_Garamond',Georgia,serif] italic text-[#D6C4AF] tracking-[0.15em] uppercase">About</span>
      ...
      {experience.length > 0 && (
        <div className="px-10 py-5">
          <div className="flex items-center gap-3 mb-4">
            <span className="text-[11px] font-['Cormorant_Garamond',Georgia,serif] italic text-[#D6C4AF] tracking-[0.15em] uppercase">Experience</span>
            <div className="flex-1 h-px bg-[#EEE6DD]" />
          </div>
      ...
      {education.length > 0 && (
        <div className="px-10 py-5">
          <div className="flex items-center gap-3 mb-4">
            <span className="text-[11px] font-['Cormorant_Garamond',Georgia,serif] italic text-[#D6C4AF] tracking-[0.15em] uppercase">Education</span>
            <div className="flex-1 h-px bg-[#EEE6DD]" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            {education.map((edu) => (
              <div key={edu.id} className="bg-white border border-[#EEE6DD] p-4">
                ...
                <p className="text-[11px] text-[#D6C4AF] mt-1 tracking-[0.1em] uppercase">{edu.graduationDate}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="px-10 py-5">
        <div className="flex items-center gap-3 mb-4">
          <span className="text-[11px] font-['Cormorant_Garamond',Georgia,serif] italic text-[#D6C4AF] tracking-[0.15em] uppercase">Expertise</span>
          <div className="flex-1 h-px bg-[#EEE6DD]" />
        </div>
          ...
          {certifications.length > 0 && (
            <div className="flex-1">
              <div className="flex flex-wrap gap-2">
                {certifications.map((c) => <Pill key={c.id}>{c.name}<span className="text-[#D6C4AF] ml-1">{c.issuer}</span></Pill>)}
              </div>
            </div>
          )}
        </div>
      ...
      {projects && projects.length > 0 && (
        <div className="px-10 py-5 pb-8">
          <div className="flex items-center gap-3 mb-3">
            <span className="text-[11px] font-['Cormorant_Garamond',Georgia,serif] italic text-[#D6C4AF] tracking-[0.15em] uppercase">Achievements</span>
            <div className="flex-1 h-px bg-[#EEE6DD]" />
          </div>
