...
const VibrantVioletTemplateComponent: React.FC<VibrantVioletTemplateProps> = ({ data }) => {
  ...
  return (
    <div className="w-full h-full min-h-[1000px] bg-white text-gray-900 shadow-xl flex">
      ...
      <div className="w-1/3 bg-gradient-to-b from-violet-700 via-purple-700 to-fuchsia-700 text-white p-8">
        <h1 className="text-4xl font-black mb-2">{personalInfo.fullName || 'Your Full Name'}</h1>
        <p className="text-xl text-violet-100 mb-6">{personalInfo.jobTitle || 'Your Job Title'}</p>
        ...
        {certifications.length > 0 && <div>
          ...
          {certifications.map(cert => <div key={cert.id} className="mb-2">
            ...
            <div className="text-xs text-violet-200">{cert.organization}, {cert.date}</div>
          </div>)}
        </div>}
      </div>
        ...
        {projects.length > 0 && <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {projects.map(proj => <div key={proj.id} className="bg-violet-50 rounded-xl p-4">
              ...
              {proj.link && <a href={proj.link} className="text-sm text-violet-600 underline">{proj.link}</a>}
