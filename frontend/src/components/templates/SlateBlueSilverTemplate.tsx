...
interface SlateBlueSilverTemplateProps {
  data: ResumeData;
}

const SlateBlueSilverTemplateComponent: React.FC<SlateBlueSilverTemplateProps> = ({ data }) => {
  const { personalInfo, experience, education, skills, projects, certifications } = data;

  return (
    <div className="w-full h-full min-h-[1000px] bg-slate-100 text-gray-900 shadow-xl">
      {/* Header */}
      <header className="bg-gradient-to-r from-slate-700 to-slate-600 text-white p-10">
        <div className="flex flex-col items-center text-center">
          <h1 className="text-4xl font-black">{personalInfo.fullName || 'Your Full Name'}</h1>
          <p className="text-xl text-slate-200 mt-2">{personalInfo.jobTitle || 'Your Job Title'}</p>
      ...
      <div className="p-10 grid grid-cols-12 gap-10">
        ...
        <div className="col-span-4 space-y-8">
          {skills.length > 0 && (
            <section className="bg-white rounded-2xl p-6 shadow-sm border-t-4 border-slate-500">
              <h3 className="text-sm font-semibold uppercase tracking-widest text-slate-700 mb-4 pb-2 border-b border-slate-200">Skills</h3>
              <div className="flex flex-wrap gap-2">
                {skills.map((skill, i) => <span key={i} className="px-3 py-1 bg-slate-200 text-slate-800 rounded-full text-xs font-medium">{skill}</span>)}
              </div>
            </section>
          )}
          ...
          {education.length > 0 && (
            <section className="bg-white rounded-2xl p-6 shadow-sm border-t-4 border-slate-500">
              <h3 className="text-sm font-semibold uppercase tracking-widest text-slate-700 mb-4 pb-2 border-b border-slate-200">Education</h3>
              {education.map(edu => (
          ...
          {certifications.length > 0 && (
            <section className="bg-white rounded-2xl p-6 shadow-sm border-t-4 border-slate-500">
              <h3 className="text-sm font-semibold uppercase tracking-widest text-slate-700 mb-4 pb-2 border-b border-slate-200">Certifications</h3>
        ...
        <div className="col-span-8 space-y-8">
          {personalInfo.summary && (
            <section className="bg-white rounded-2xl p-6 shadow-sm border-l-4 border-slate-500">
          ...
          {projects.length > 0 && (
            <section className="bg-white rounded-2xl p-6 shadow-sm border-l-4 border-slate-500">
              ...
              <div className="grid grid-cols-2 gap-4">
                {projects.map(proj => (
                  <div key={proj.id} className="bg-slate-100 rounded-lg p-4">
                    <h4 className="font-semibold text-slate-900">{proj.name}</h4>
                    {proj.link && <a href={proj.link} className="text-xs text-slate-600 underline">{proj.link}</a>}
...
export default React.memo(SlateBlueSilverTemplateComponent);
