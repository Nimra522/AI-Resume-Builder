...
interface SapphireEleganceTemplateProps {
  data: ResumeData;
}

const SapphireEleganceTemplateComponent: React.FC<SapphireEleganceTemplateProps> = ({ data }) => {
  const { personalInfo, experience, education, skills, projects, certifications } = data;

  return (
    <div className="w-full min-h-[1000px] bg-slate-50 text-slate-800 shadow-xl">
      {/* Header */}
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 text-white px-10 py-12 border-b-4 border-amber-500">
        <div className="flex items-center gap-8">
          {personalInfo.photoUrl ? (
            <div className="w-36 h-36 rounded-full border-4 border-amber-500 shadow-xl overflow-hidden flex-shrink-0">
      ...
      <div className="grid grid-cols-12 gap-8 px-10 py-10">
        ...
        <div className="col-span-4 space-y-8">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
            <h3 className="text-sm font-bold uppercase tracking-widest text-slate-700 mb-4 pb-2 border-b border-amber-200">Contact</h3>
          ...
          {skills.length > 0 && (
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
              <h3 className="text-sm font-bold uppercase tracking-widest text-slate-700 mb-4 pb-2 border-b border-amber-200">Skills</h3>
              <div className="flex flex-wrap gap-2">
                {skills.map((skill, i) => (
                  <span key={i} className="px-3 py-1.5 bg-gradient-to-r from-slate-100 to-amber-100 text-slate-800 border border-amber-200 rounded-full text-xs font-semibold">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          {education.length > 0 && (
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
              <h3 className="text-sm font-bold uppercase tracking-widest text-slate-700 mb-4 pb-2 border-b border-amber-200">Education</h3>
              <div className="space-y-4 text-sm">
                {education.map((edu) => (
                  <div key={edu.id}>
                    <div className="font-bold text-slate-900">{edu.school}</div>
                    <div className="text-amber-700">{edu.degree}</div>
          ...
          {certifications.length > 0 && (
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
              <h3 className="text-sm font-bold uppercase tracking-widest text-slate-700 mb-4 pb-2 border-b border-amber-200">Certifications</h3>
              <div className="space-y-3 text-sm">
          ...
          {projects.length > 0 && (
            <div className="bg-white rounded-2xl p-7 shadow-sm border border-slate-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {projects.map((proj) => (
                  <div key={proj.id} className="bg-gradient-to-br from-slate-50 to-amber-50 p-5 rounded-xl border border-slate-200">
                    <h4 className="font-bold text-slate-900">{proj.name}</h4>
                    {proj.link && (
                      <a href={proj.link} className="text-xs text-amber-700 hover:text-amber-600 underline block mt-1" target="_blank" rel="noopener noreferrer">
                        {proj.link}
                      </a>
                    )}
...
export default React.memo(SapphireEleganceTemplateComponent);